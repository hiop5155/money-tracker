        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                ...
                >
                    ...
                    {dayTotal > 0 && <div className="text-xs text-right font-bold text-red-500 dark:text-red-400 truncate w-full">${dayTotal}</div>}
                    {dayExpenses.length > 0 && (
                        <div className="flex gap-0.5 flex-wrap content-end">
                            {dayExpenses.slice(0, 3).map((ex, idx) => (
                                <div key={idx} className="w-1.5 h-1.5 rounded-full bg-blue-400"></div>
                            ))}
                        </div>
                    )}
                </div>
            ...
            <div className={`flex-1 rounded-xl shadow-sm overflow-hidden flex flex-col ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
                ...
                >
                    <div className="flex justify-between items-center pr-2 border-r border-gray-300 dark:border-slate-600">
                        ...
                        <span className="font-medium">{formatCurrency(budgets.monthly)}</span>
                    </div>
                    <div className="flex justify-between items-center pl-2">
                        <span className="text-gray-500 dark:text-slate-400">本月支出:</span>
                        <span className={monthlyTotal > budgets.monthly ? 'text-red-500 font-bold' : 'text-green-500 font-bold'}>
                    ...
                    <div className="flex justify-between items-center pr-2 border-r border-gray-300 dark:border-slate-600 pt-2 border-t dark:border-t-slate-700">
                        <span className="text-gray-500 dark:text-slate-400">年度預算:</span>
                        <span className="font-medium">{formatCurrency(budgets.yearly)}</span>
                    </div>
                    <div className="flex justify-between items-center pl-2 pt-2 border-t border-gray-300 dark:border-t-slate-700">
                        ...
                        <span className={yearlyTotal > budgets.yearly ? 'text-red-500 font-bold' : 'text-green-500 font-bold'}>
                ...
                <div className={`rounded-xl shadow-sm p-4 flex-1 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {selectedDateExpenses.length === 0 ? (
                            <p className={`text-center py-8 text-sm ${isDark ? 'text-slate-500' : 'text-gray-400'}`}>今日尚無紀錄</p>
                        ) : (
                                ...
                                <div key={item.id} className={`flex flex-col p-3 rounded-lg group ${isDark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                                    <div className="flex justify-between items-start">
                                        <div className="flex flex-col">
                                            ...
                                            <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-400'}`}>{item.note || '無備註'}</span>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className={`font-bold ${isDark ? 'text-slate-200' : 'text-gray-800'}`}>${item.amount}</span>
                                            <button
